        ...
        <div class="conten-box mt-14 border-t border-slate-100 dark:border-slate-700 -mx-6 px-6 pt-6">
            <div v-if="stepNumber === 0" class="px-5">
                ...
                <div class="grid lg:grid-cols-4 grid-cols-2">
                    ...
                    <div>
                        <Select placeholder="Select an option" :options="dropdown_store.dropdownlist.modules"
                            v-model="report.form.modules" @option:selected="SelectModule"
                ...
                <div class="grid lg:grid-cols-4 grid-cols-2">
                    ...
                    <div>
                        <Select placeholder="Select an option" :options="report.relation_module"
                            v-model="report.form.related_module" :reduce="option => option.value"
                            :selectable="(option) => report.form.related_module.length < 2 " multiple />
                    </div>
                </div>
            </div>
            <div v-if="stepNumber === 1" class="px-5">
                <Card title="Choose List conditions">
                    ...
                    <div class="grid grid-cols-9 gap-8 mt-4" v-for="(item,i) in report.form.filter" :key="i">
                        ...
                        <div class="col-start-4 col-span-2">
                            <Select placeholder="Select an option" :options="operator"
                                v-model="report.form.filter[i].operator" :reduce="option => option.value" />
                        </div>
                        <div class="col-start-6 col-span-3">
                            <Textinput v-if="report.form.filter[i].type == 'text' " placeholder=""
                                v-model="report.form.filter[i].value" />
                            <Textarea v-else-if="report.form.filter[i].type == 'textarea' "
                                v-model="report.form.filter[i].value" />
                            <flat-pickr v-else-if="report.form.filter[i].type == 'date' " class="form-control"
                                placeholder="yyyy-mm-dd" :config="{ dateFormat:'Y-m-d' }"
                                v-model="report.form.filter[i].value" />
                            <flat-pickr v-else-if="report.form.filter[i].type == 'time' " class="form-control"
                                placeholder="HH:mm:00"
                                :config="{ enableTime: true, noCalendar: true, dateFormat: 'H:i:00',time_24hr:true,minuteIncrement:1 }"
                                v-model="report.form.filter[i].value" />
                            <Select placeholder="Select an option" v-else-if="report.form.filter[i].type == 'dropdown' "
                                :options="dropdown_store.dropdownlist_data" />
                            <Textinput v-else :type="report.form.filter[i].type" placeholder=""
                                v-model="report.form.filter[i].value" />
                        </div>
                    ...
                    <Button icon="heroicons-outline:plus" text="Add Condition" btnClass="btn-success mr-2 py-2 mt-3"
                        @click="andCondition" />
                </Card>
            </div>
            <div v-if="stepNumber === 2" class="px-5">
                <Card title="Select chart type">
                    <div class="fromGroup relative mt-5">
                        ...
                        <Select placeholder="Select an option" :options="chart_type"
                            v-model="report.form.chart.type"
                            :reduce="option => option.value" />
                    </div>
                    ...
                    <div class="fromGroup relative mt-5">
                        ...
                        <Select placeholder="Select an option" :options="data_fields"
                            multiple
                            v-model="report.form.chart.dataset"
                            :reduce="option => option.value" />
                    </div>
            ...
            <div class="mt-10" :class="stepNumber > 0 ? 'flex justify-between' : ' text-right'">
                ...
                <Button @click="submit" :text="stepNumber !== this.steps.length - 1 ? 'next' : 'Save and Generate'"
                    btnClass="btn-dark" />
            </div>
        </div>
    </Card>
...
export default {
    ...
    methods: {
        clear2(){
            ...
            report.form.chart.type = "";
            report.form.chart.dataset = [];
            report.form.chart.group_by = "";
            report.form.filter = 
                [
                    {
                        field: "",
        ...
        submit() {
           ...
           if(stepNumber.value == 0){
            ...
            if(report.form.selected_column.length == 0 && type == 'list'){
