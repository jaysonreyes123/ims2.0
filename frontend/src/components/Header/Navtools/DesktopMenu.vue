<template>
  <ul>
    <li
      ...
      "
    >
      <router-link :to="item.link" v-if="!item.child && !item.megamenu && module_page[item.name] == 1 ">
      ...
      <ul class="sub-menu" v-if="item.child">
        ...
        >
          <router-link :to="childitem.childlink" v-if="!childitem.submenu && module_page[childitem.name]">
...
<script>
import { topMenu } from "../../../constant/menu.js";
